...
namespace FelpoII.Core.Pieces
{
    ...
    {
        public Queen(Side owner, IChessBoard board,int cell)
        ...
        {
            int dir1 = DiagAttackLookup[Square.Ox88Dist(square, HomeSquare)];
            int dir2 = StraightAttackLookup[Square.Ox88Dist(square, HomeSquare)];

            if (dir1 != 0)
            {
                if (false == AttackPathFree(dir1, square))
                    return false;
            }
            if (dir2 != 0)
            {
                if (false == AttackPathFree(dir2, square))
                    return false;
            }
        ...
        protected override int OnGetBlockingMoves(int start, int[] moves, int[] ray, int rayLen)
        {
            ...
            for (int i = 0; i < rayLen; ++i)
            {
                ...
                dir = StraightAttackLookup[Square.Ox88Dist(square, HomeSquare)];
                if ((PinStatus == PinStatus.None || PinCompatible(dir)) &&
                    AttackPathFree(dir, square) )
                {
                    moves[start++] = MovePackHelper.Pack(HomeSquare,square);
        ...
        { 
            for( int rank=0;rank<8;rank++ )
                ...
                {
                    ZKeysWhite[rank<<4|file] = Random64[GetZKeyIndex(PieceType.Queen,rank,file,Side.White)];
                    ZKeysBlack[rank << 4 | file] = Random64[GetZKeyIndex(PieceType.Queen, rank, file, Side.Black)];
                }
        }
    }
}
